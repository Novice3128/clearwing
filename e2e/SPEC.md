...
> 版本：v1（2026-09-18，由六輪實測＋四代理審查沉澔回溯標準化）。
...
8. **清理 SOP（每 run 收尾，斷言化）**：僅移除本輪記錄的 `clearwing-kali-<sid>` 容器（**禁 name-filter 全清**——docker 為全域共用）；`/tmp/report_*`/`kerbrute*` 淨；8787/8898 釋放；tmux 無（已自動斷言）；產物 `api_key` grep＝0（含 query 形式 `?api_key=`）、截圖 strings＝0（已自動斷言）；8899 health 200 且 pid 未變。**成員 8899 的 webui log 掃描屬 R3 人工步驟**——live log redirect 目前寫入已刪除 inode 且 WS-accepted 行不經 redact filter（見 gh/unauth-endpoints-report.md 附錄），修復前自動掃不可行。

## 3. 層級與觸發（事件驅動，非每 PR 盲跑）

| 層 | 觸發 | 成本 | 內容摘要（細節=suite.yaml） |
|---|---|---|---|
| Tier0 verify | 每 run 內建 | $0 | 版本探針（pid start/cwd vs HEAD＋web-api.md commit，抓 pre-merge 舊碼）、health/401、金鑰等值（去換行）、bak 殘留掃、埠佔用 abort、doctor/models 組態漂移、狀態＋.82 目標端快照 |
| Quick | 8899 HEAD 變更且 diff 觸及熱區（ui/web、llm、observability、reporting、agent/tools） | ≈$0.2–0.4 | zero-LLM 探針、fc 審批任務（status/重複幀/遲到幀）、**stop-frame**、**watchdog 低帽觸發**、warm×1（HUD 頁面渲染）、save_report 鏈、report -s、refuse×1、裸 base_url 404 探針、定向 pytest |
| Full | 發版/里程碑 | ≈$3–5 | 五軸全量＋flag 基線（9 面）＋R3 抽核＋audit 缺記檢查＋混沌 4+2 式（含 429、deny 路徑）＋APPROVE_DELAY 變體＋baseline diff（±30% 帶） |
| Deep-cold | 排程/需要冷啟動數據 | ≈$3–5 | 隔離 home（**起跑斷言空記憶**＋首 recall 無回灌）＋煙測＋與暖跑對照＝冷啟動係數 |
| Deep-fallback | **明確核准**（--approve-fallback） | ≈$0.1 | real-config 手術＋8898 承載：全拒/回切/slow-fail 三場景 |
...
- **硬門（任一觸發＝FAIL）**：相鄰重複 agent_message 對>0；終態 complete 後遲到幀>0；審批閉包破；**終態 complete 落在未執行審批之上（D1/#29 偵測器）**；快取命中=0 或 <80%（Full 大 session）；**對帳差>0.5% 或 audit 缺記 llm_call**；場景 `expect:` 全部鍵（approvals 下限/cancelled_turn/watchdog 觸發/complete_status/errors）；flag faces 超 Full 基線（9）。`skipped` 場景不計 FAIL（severity=skip）。
- **比例門**：audit×PRICING 對帳 ≤0.5%；cache ≥80%（Full）；HUD≡報告（頁面渲染，精確到分）。
- **趨勢門**：時長/成本 vs 前次 Full ±30% 帶（首輪建立基線）。
